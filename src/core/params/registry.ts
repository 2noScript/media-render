import type { ParamDefinition, ParamValue, ParamValues } from "@/core/params";
...
elementParamRegistry.register({
	key: "audio",
	definition: audioElementParams,
});
elementParamRegistry.register({
	key: "effect",
	definition: [],
});
const transitionElementParams: ElementParamDefinition[] = [
	{
		key: "duration",
		label: "Duration",
		type: "number",
		...
		step: 0.1,
		keyframable: false,
		read: ({ element }) => element.duration,
		write: ({ element, value }) => ({
			...element,
			duration: typeof value === "number" ? value : element.duration,
		}),
	},
];
...
elementParamRegistry.register({
	key: "transition",
	definition: transitionElementParams,
});
