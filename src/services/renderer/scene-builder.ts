...
import { DEFAULT_BACKGROUND_BLUR_INTENSITY } from "@/lib/background/defaults";
import {
...
export function buildScene({
	...
	const bg = background ?? { type: "color", color: "transparent" };
	...
	if (bg.type === "blur") {
		const blurNodes = buildBlurBackgroundNodes({
			track: mainTrack,
			mediaMap,
			blurIntensity: bg.blurIntensity ?? DEFAULT_BACKGROUND_BLUR_INTENSITY,
		});
		for (const node of blurNodes) {
			rootNode.add(node);
		}
	} else if (bg.type === "color" && bg.color !== "transparent") {
		rootNode.add(new ColorNode({ color: bg.color }));
	}
